...
const SignUp = () => {
  ...
  const handleSubmit = () => {
    axios
      ...
      .then((res) => {
        console.log("Success:", res.data); // Logs the server response
      })
      .catch((err) => {
        console.log(err);
      });
    ...
    <>
      <div className=" w-full h-full flex justify-center items-center bg-gradient-to-b from-red-500  ">
        <div className="bg-white rotate-6 max-sm:w-80 max-sm:rounded-xl max-sm:rotate-0 shadow-2xl shadow-red-700 hover:rotate-0 transition-all duration-500">
          <div className="bg-white w-fit max-sm:w-80 max-sm:rounded-xl max-sm:rotate-0  p-4 -rotate-6   hover:rotate-0 transition-all duration-500">
